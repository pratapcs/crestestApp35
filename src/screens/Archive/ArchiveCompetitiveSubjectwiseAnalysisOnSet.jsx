...
const ArchiveCompetitiveSubjectwiseAnalysisOnSet = props => {
  const dispatch = useDispatch();
  const { examType = '', set_no = '', subType = '', class_id = '' } = props?.route?.params;
  ...
  const getcompetitiveSubjectwisecomparison = useSelector(
    state => state.archivePerformance.getcompetitiveSubjectwisecomparison,
  );
  const getcompetitiveNonverbalcomparison = useSelector(
    state => state.archivePerformance.getcompetitiveNonverbalcomparison,
  );
  const wheredoyoustandCompetitive = useSelector(
    state => state.archivePerformance.wheredoyoustandCompetitive,
  );
  ...
  useEffect(() => {
    if (
      ...
      dispatch(
        whereDoYouStandCompetitiveDataDetails(examType, subType, set_no, class_id, props),
      );
  ...
  }, [examType, set_no, subType, class_id]);
  ...
  return (
    <KeyboardAvoidingView
      ...
      />
      <HeaderComponent
        headerName={`Archive ${examType} Competitive Subjectwise Analysis On Set`}
...
export default ArchiveCompetitiveSubjectwiseAnalysisOnSet;
