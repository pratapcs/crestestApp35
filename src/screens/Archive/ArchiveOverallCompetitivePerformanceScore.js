...
const ArchiveOverallCompetitivePerformanceScore = props => {
  ...
  const compititiveSetwiseScore = useSelector(
    state => state.archivePerformance.compititiveSetwiseScore,
  );
  const getcompetitiveSubjectAvgscore = useSelector(
    state => state.archivePerformance.getcompetitiveSubjectAvgscore,
  );

  const competitiveOverAllPerformance = useSelector(
    state => state.archivePerformance.competitiveOverAllPerformance,
  );
  useEffect(() => {
    if (examType !== '') {
      dispatch(getCompetitiveOverAllPerformanceData(examType, props.route.params.class_id, props));
      dispatch(
        getCompareScholasticCompetitiveDataDataDetails(examType, 0, props.route.params.class_id, props),
      );
      dispatch(getCompetitiveSetwiseScoreDataDetails(examType, props.route.params.class_id, props));
      dispatch(getcompetitiveSubjectAvgscoreDataDetails(examType, 0, props.route.params.class_id, props));
      return () => {
        // dispatch(
  ...
  return (
    <KeyboardAvoidingView
      ...
      />
      <HeaderComponent
        headerName={`Archive ${examType} Performance Score`}
                ...
                <CompetitivePerformanceActivityCard
                  onPressSet={setNo => {
                    props.navigation.navigate('nonAuthScenes', {
                      screen: 'ArchiveSubjectWiseCompetitiveScore',
                      params: {
                        examType: examType,
                        set_no: setNo.slice(3),
                        class_id: props.route.params.class_id,
                      },
                    });
                  }}
...
export default ArchiveOverallCompetitivePerformanceScore;
